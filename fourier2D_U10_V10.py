...
#        output,_,_,_,_,_,_ = net(input_batch.cuda())
    ...
    # print statistics
    loss = spectral_loss_mv(output, label_batch.cuda(),wavenum_init,wavenum_init_ydir,lamda_reg,(torch.tensor(ocean_grid)).cuda())
#        loss = ocean_loss(output, label_batch_crop.cuda(),(torch.tensor(ocean_grid)).cuda())
    loss.backward()
    optimizer.step()
    output_val = PECstep(net,(psi_test_input_Tr_torch_norm_level1[0:num_samples,:,0:Nlat,0:Nlon].reshape([num_samples,len(varlist),Nlat,Nlon])).permute(0,2,3,1))
    val_loss = spectral_loss_mv(output_val, (psi_test_label_Tr_torch_norm_level1[0:num_samples,:,0:Nlat,0:Nlon].reshape([num_samples,len(varlist),Nlat,Nlon])).permute(0,2,3,1).cuda(),wavenum_init,wavenum_init_ydir,lamda_reg,(torch.tensor(ocean_grid)).cuda())
...
torch.save(net.state_dict(), path_outputs+'DTAS_BNN_FNO2D_PECstep_U10_V10_ocean_MV_spectral_loss_modes_'+str(modes)+'_wavenum'+str(wavenum_init)+'lead'+str(lead)+'.pt')
...
# hdf5storage.write(matfiledata, '.', path_outputs+'predicted_FNO_2D_PECstep_MSSHF_MSLHF_ocean_spectral_modes_'+str(modes)+'train_wavenumber'+str(wavenum_init)+'lead'+str(lead)+'lambda_'+str(lamda_reg)+'.mat', matlab_compatible=True)
